...
/**
 * Input specifies one input of a transaction % given by the sender given by a
...
public class Input {
	...
	/**
	 * If we have a Wallet covering the sender and an outputList
	 *
	 * then we can compute the signature by signing the transaction to be signed
	 * consisting of the public key and input amount and the output list using the
	...
	/**
	 * Create the corresponding element of InputUnsigned which is obtained by
	...
	/**
	 * Task 1 Check the signature is correct for a given OutputList. (The OutputList
	 * is needed to determine the message to be signed which consists of the sender,
	 * amount, and the public keys and amounts for each output. It is computed in
	 * the method getMessageToSign of InputList.java)
	 * 
	 * This can be done by getting the underlying InputUnsigned and executing the
	 * method checkSignature for it referring to the OutputList and the signature
	 * which is a field of Input
	 * 
	...
	public boolean checkSignature(OutputList outList) {
		return this.toInputUnsigned().checkSignature(outList, this.getSignature());
	}
