...
/**
 * InputList defines a list of Inputs of a transaction
 *
 */
...
public class InputList {
	...
	/**
	 * If we have a Wallet covering the sender and an outputList then we can compute
	 * the signature by signing the transaction to be signed consisting of the
	 * public key and input amount and the outputList using the private key of the
	 * sender
	 */
	...
	public InputList(PublicKey sender1, int amount1, byte[] signature1, PublicKey sender2, int amount2,
			byte[] signature2) {
		inputList = new ArrayList<Input>();
	...
	public InputList(String sender1, int amount1, String sender2, int amount2, OutputList outputList, Wallet wallet)
			throws NoSuchAlgorithmException, SignatureException, InvalidKeyException {
		inputList = new ArrayList<Input>();
	...
	/**
	 * as before but for 2 users using outputList and wallet, this time senders are
	 * given by public keys
	 **/

	public InputList(PublicKey sender1, int amount1, PublicKey sender2, int amount2, OutputList outputList,
	...
	/**
	 * when checking that InputList can be deducted from an ledger it is not enough
	 * to check that each single item can be deducted since for the same sender
	 * several items might occur
	 * 
	 * in order to check that the InputList can be deducted we first create an
	 * ledger containing for each user the sum of amounts to be deducted
	 *
	 * then we can check whether each entry in the original ledger is greater the
	 * sum of items for each user to be deducted
	 */
	...
	/**
	 * function to print all items in the InputList in the form word1 <sender> word2
	 * <amount>
	 *
	 * 
	...
	/**
	 * Task 2 Check the signatures of each element of the InputList is correct
	 * w.r.t. the OutputList given You can make use of the checkSignature method of
	 * individual Input (elements of Input).
	 * 
	 * In order for the code to compile it has been defined as True but that should
	 * be adapted.
	 **/
	...
	public boolean checkSignature(OutputList outList) {
		...
		for (Input input : this.inputList) {
			if (valid) {
				valid = input.checkSignature(outList);
			}
		}
		return valid;
	}
	...
	/**
	 * Generic Test cases, providing a headline printing out the InputList and
	 * printing out the sum of amounts in the InputList
	 */
	...
	public static void test() throws NoSuchAlgorithmException, SignatureException, InvalidKeyException {
		...
		InputList l = new InputList(pubKeyA, 10, signedMessage1, pubKeyB, 20, signedMessage1);
