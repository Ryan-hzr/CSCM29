...
/**
 * Transaction consisting of a list of inputs and a list of outputs
 */
...
public class Transaction {
	...
	/*
	 * Task 3 check all signatures are valid
	 * 
	...
	public static void test() throws NoSuchAlgorithmException, SignatureException, InvalidKeyException {
		...
		Transaction tx = new Transaction(new InputList(), new OutputList());
		tx.testCase("Transaction null to null", pubKeyMap);
		tx = new Transaction(new InputList(pubKeyA, 10, signedMessage1), new OutputList(pubKeyB, 5));
		tx.testCase("Transaction Alice 10  to Bob 5", pubKeyMap);

		tx = new Transaction(new InputList(pubKeyA, 5, signedMessage1), new OutputList(pubKeyB, 10));
